...
public class Grid {
	...
	public void checkRows() {
		// check along the height of the grid
		for (int i = Grid.HEIGHT - 1; i >= 0; --i) {
			boolean full = true;
			
			// check along the width of the grid
			for (int j = 0; j < Grid.WIDTH; ++j)
				// if the row in the grid is not full
				// goes into the if statement
				if (!isSet(i,j)) {
				// set variable full to false
				full = false;
				// break out of the loop
				break;
				}
			
			// if the row in the grid is full
			// goes into the if statement
			if (full) 
				
				// moves the pieces on the top of the removed row
				// down one row
	            for (int k = i; k > 0; --k) 
	                for (int j = 0; j < Grid.WIDTH; ++j)
