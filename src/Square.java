...
public class Square {
	...
	public boolean canMove(Direction direction) {
		...
		// if the LEFT direction is blocked, we can't move
		case LEFT:
			if (col == 0 || grid.isSet(row, col - 1))
				move = false;
			break;
				
		// if the RIGHT direction is blocked, we can't move
		case RIGHT:
	...
	public void move(Direction direction) {
		if (canMove(direction)) {
			...
			//  if the can move to the RIGHT direction 
			//	add col by 1 which moves
			...
			//  if the can move to the RIGHT direction 
			//	subtraction col by 1 which moves 
	...
	// use the s as the center, s.getRow()
	// in abstract piece, this s is the the square[1]
	public void rotate(Square s) {
		int yDistance = row - s.getRow();
		int xDistance = col - s.getCol();
		int curRow = s.getRow() + xDistance;
		int curCol = s.getCol() - yDistance;
	...
	public boolean canRotate(Square s) {
		int yDistance = row - s.getRow();
		int xDistance = col - s.getCol();
		int curRow = s.getRow() + xDistance;
		int curCol = s.getCol() - yDistance;
		...
		// check for boundary
		if (curCol < 0 || curRow >= Grid.HEIGHT || curRow < 0 || curCol >= Grid.WIDTH) {
            rotate = false;
		} else {
			// check whether the place for square that is rotated is empty
			if (grid.isSet(curRow, curCol))
				rotate = false;
			}
		return rotate;
	}
